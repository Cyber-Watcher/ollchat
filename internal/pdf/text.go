...
// Метки указателя, вклеенные в текст.
//
// Некоторые издательские конвейеры ставят в текстовый слой невидимые читателю
// якоря предметного указателя — «idx_07e7f06f». В файле это обычный текст,
// нарисованный по месту, поэтому в разбор он попадает наравне со словами
// и склеивается с соседними: «Bash oridx_07e7f06fidx_1e43d732 Python».
//
// Замер на библиотеке владельца (23.08.2026): такие метки нашлись в 12 книгах
// из 221, всего 12 720 штук в 7 110 кусках — 2.6% библиотеки. Вред тройной:
// слово рядом с меткой перестаёт находиться поиском по словам, смысл куска
// портится у эмбеддинга, а при построении графа модель принимает метку
// за понятие — в графе завелось 345 записей с «idx_».
//
// Проверено, что это не наш дефект разбора: pdftotext на той же странице
// печатает те же метки. Поэтому убираем их как заведомый мусор, а не чиним
// раскладку.
//
// Правило узкое намеренно: «idx_» и ровно восемь шестнадцатеричных цифр.
// Похожее имя переменной в примере кода потерять жаль, но такая потеря куда
// дешевле, чем мусор в каждом четвёртом куске задетых книг.
var reIndexAnchor = regexp.MustCompile(`idx_[0-9a-f]{8}`)

func dropIndexAnchors(s string) string {
	if !strings.Contains(s, "idx_") {
		return s
	}
	return reIndexAnchor.ReplaceAllString(s, "")
}
