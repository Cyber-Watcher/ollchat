...
func TestDetectFile(t *testing.T) {
	cases := []struct {
		...
		{"note.txt", []byte("обычный текст"), KindText},
		{"guide.md", []byte("# Заголовок"), KindMarkdown},
...
func TestReadRejectsOtherFiles(t *testing.T) {
	// .txt и .md читаются как документы намеренно (см. text.go), а вот
	// файл с чужим расширением и без подписи документом не считается.
	if _, err := Read(writeTemp(t, "note.bin", []byte("текст")), 0); err == nil {
		t.Fatal("обычный файл прочитан как документ")
	}
	if _, err := Read(writeTemp(t, "code.go", []byte("package main")), 0); err == nil {
		t.Fatal("исходник прочитан как документ")
	}
}
