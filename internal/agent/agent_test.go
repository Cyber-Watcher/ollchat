...
func fakeRunner(t *testing.T, client Chatter) *Runner {
	t.Helper()
	root := t.TempDir()
...
// Отрицательный предел означает «без ограничения»: цепочка идёт, пока модель
// просит инструменты. Проверяем на сервере, который отдаёт вызов инструмента
// сорок раз подряд — много больше умолчания в 25.
func TestUnlimitedIterations(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("Content-Type", "application/x-ndjson")
		if n <= 40 {
			fmt.Fprint(w, `{"message":{"role":"assistant","tool_calls":[{"function":{"name":"list_dir","arguments":{"path":"."}}}]},"done":false}`+"\n")
	...
	reg, err := tools.NewRegistry([]string{tools.NameListDir}, tools.Options{Sandbox: sb, MaxOutputKB: 64})
	if err != nil {
		t.Fatal(err)
	}
	...
	r := plainRunner(ollama.New(srv.URL, 0, 0, nil), 0)
	r.MaxIterations = -1
	r.Tools, r.ToolsSupported = reg, true
	r.Guard = permissions.NewGuard(set, sb, permissions.ModeYolo)
	...
	if len(errs) > 0 {
		t.Fatalf("ход оборван при -1: %v", errs[0])
	}
	if got := atomic.LoadInt32(&calls); got <= 25 {
		t.Fatalf("цепочка остановилась на %d обменах — предел всё ещё действует", got)
	}
}
