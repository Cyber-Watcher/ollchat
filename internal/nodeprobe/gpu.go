...
//
// Имя программы — ЗАПАСНОЙ признак принадлежности службе (`ollama`,
// `ollama_llama_server`). Главный — родство: свои счётчики моделей Ollama
// зовёт `llama-server`, и по имени они выглядели чужими, из-за чего 12.09.2026
// наблюдатель объявил чужой работой нашу же сборку. Родство размечается после
// сбора раздела службы — см. `markOwnGPUProcs` в own.go. Искать по строке
// команды нельзя: под такой шаблон попадает и сам ищущий, и чужая программа
// с похожим именем.
