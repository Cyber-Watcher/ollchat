...
// TestCompoundCommandNeedsConfirmation — **правило изменено 31.08.2026
// по решению владельца.**
//
// Было: составная команда спрашивала всегда, даже когда обе части разрешены
// правилами. На живой работе это оказалось вредно: `grep … | head -50; ls …` —
// три чтения, каждое разрешено, а окно подтверждения всё равно вставало
// поперёк. Человек, отвечающий «да» по десять раз в час на заведомо
// безобидное, перестаёт читать, что ему показывают, — и однажды подтвердит
// настоящую беду.
//
// Стало: склейка разрешённых частей разрешена, если склеены они безобидно.
// Что НЕ ослаблено (проверяется в pipeline_test.go):
//   - deny разбирает каждую часть при любой склейке;
//   - перенаправление, подстановка, фон и подоболочка спрашивают всегда,
//     сколько бы правил ни стояло: `cat файл > /etc/passwd` состоит из одной
//     разрешённой части и переписывает системный файл;
//   - одна неразрешённая часть возвращает вопрос про всю строку;
//   - сеансовые разрешения на составные команды по-прежнему не действуют.
func TestCompoundCommandNeedsConfirmation(t *testing.T) {
	g, _ := newTestGuard(t, ModeSafe)

	// Части разрешены правилами Bash(go build:*) и Bash(ls:*), склейка — `&&`.
	res := g.Check(Request{Kind: KindBash, Target: "go build ./... && ls -la"})
	if res.Decision != DecisionAllow {
		t.Errorf("склейка разрешённых частей должна проходить без вопроса, получено %v (%s)",
			res.Decision, res.Reason)
	}

	// А неразрешённая часть возвращает вопрос про всю строку.
	res = g.Check(Request{Kind: KindBash, Target: "ls -la && make install"})
	if res.Decision != DecisionAsk {
		t.Errorf("неразрешённая часть обязана вернуть вопрос, получено %v (%s)",
			res.Decision, res.Reason)
	}
}
...
// TestFixedTargetNeedsNoConfirmation — цель, заданную настройкой, не спрашивают.
//
// Поиск по книгам и графу всегда читает один и тот же каталог из конфига,
// и модель на него повлиять не может. Но запрет обязан работать и здесь:
// иначе «не спрашивать» однажды превратится в «нельзя запретить».
func TestFixedTargetNeedsNoConfirmation(t *testing.T) {
	g, root := newTestGuard(t, ModeSafe)
	kbDir := filepath.Join(root, "..", "kb")

	fixed := Request{Kind: KindRead, Target: kbDir, Tool: "kb_search", Fixed: true}
	if res := g.Check(fixed); res.Decision != DecisionAllow {
		t.Fatalf("подтверждение запрошено на цель из настроек: %+v", res)
	}
	// Та же цель, но выбранная моделью, проходит обычным порядком.
	free := Request{Kind: KindRead, Target: kbDir, Tool: "read_file"}
	if res := g.Check(free); res.Decision == DecisionAllow {
		t.Fatalf("свободная цель разрешена без правила: %+v", res)
	}

	// Запрет сильнее: тот же вызов под правилом deny закрыт.
	sb, err := NewSandbox(root, false, false, 512)
	if err != nil {
		t.Fatal(err)
	}
	set, err := Compile(nil, nil, []string{"Read(" + kbDir + "/**)"}, sb.Root())
	if err != nil {
		t.Fatal(err)
	}
	denied := NewGuard(set, sb, ModeSafe)
	req := Request{Kind: KindRead, Target: filepath.Join(kbDir, "collections"),
		Tool: "kb_search", Fixed: true}
	if res := denied.Check(req); res.Decision != DecisionDeny {
