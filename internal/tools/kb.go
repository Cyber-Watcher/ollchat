...
func (t *kbSearchTool) Plan(args map[string]any) (*Plan, error) {
	...
	title := fmt.Sprintf("%s(%s)", NameKBSearch, shorten(query, 50))
	if name != "" {
		title = fmt.Sprintf("%s(%s, %s)", NameKBSearch, name, shorten(query, 40))
	}
	return &Plan{
		Tool: NameKBSearch,
		...
		Req:   permissions.Request{Kind: permissions.KindRead, Target: t.opts.KBDir, Tool: NameKBSearch},
		Title: title,
...
func (t *kbSearchTool) run(ctx context.Context, name, query, book string, topK int) (string, error) {
	...
	if t.opts.KBTopK > 0 {
		opt.TopK = t.opts.KBTopK
	}
	if t.opts.KBMaxPerBook > 0 {
		opt.MaxPerDoc = t.opts.KBMaxPerBook
	}
	if topK > 0 {
		opt.TopK = min(topK, 20)
	}
	if book != "" {
		opt.Docs = booksMatching(coll, book)
		if len(opt.Docs) == 0 {
			return fmt.Sprintf("В коллекции %q нет книг, название которых содержит %q.", coll.Name(), book), nil
		}
	}

	// Смысл участвует, когда он посчитан и есть чем векторизовать запрос;
	// иначе SearchWith возвращает ровно то же, что и поиск по словам.
	opt.Semantic = t.opts.Semantic
	opt.MinCosine = t.opts.MinCosine
	opt.SemanticWeight = t.opts.SemanticWeight
	hits, err := coll.SearchWith(ctx, query, opt, t.opts.embedder())
	if err != nil {
		return "", err
	}
	if len(hits) == 0 {
...
func booksMatching(coll *kb.Collection, want string) []uint32 {
...
func (t *kbReadTool) Plan(args map[string]any) (*Plan, error) {
	...
	return &Plan{
		Tool:  NameKBRead,
		Req:   permissions.Request{Kind: permissions.KindRead, Target: t.opts.KBDir, Tool: NameKBRead},
...
func (o Options) collection(name string) (*kb.Collection, error) {
	if o.KB == nil {
