...
func runCommand(ctx context.Context, command string, opts Options, timeout time.Duration) (string, error) {
	...
	pw.Close()
	// Читающий конец закрывается в любом случае: на обычном пути — здесь,
	// на пути с выжившим потомком — раньше, чтобы снять горутину чтения.
	// Повторное закрытие безопасно, ошибка от него ничего не значит.
	...
	// Сперва ждём горутину чтения и только потом закрываем читающий конец.
	//
	// **Гонка, пойманная 05.09.2026 на второй машине** (4 ядра, шла сборка):
	// раньше `pr.Close()` стоял перед ожиданием, и если горутину чтения ещё
	// ни разу не поставили на процессор, `io.Copy` получал «file already
	// closed» и отдавал пустой буфер. Наружу это выглядело как успешная
	// команда без вывода: «Код возврата: 0 … (команда не вывела ничего)» —
	// то есть молчаливая потеря результата. На двенадцати ядрах не
	// воспроизводилось: горутина всегда успевала первой.
	//
	// Закрывать `pr` всё равно надо: выживший потомок держит пишущий конец,
	// и без закрытия чтение висит вечно — ради этого закрытие и появилось.
	// Поэтому порядок такой: ждём `readGrace` (обычному пути хватает,
	// EOF приходит сразу после выхода потомка), и только по истечении
	// закрываем конец и забираем, что успело прочитаться.
	var text string
	select {
	case text = <-outCh:
	case <-time.After(readGrace):
		_ = pr.Close() // снимает горутину чтения, дальше она отдаёт буфер сама
		select {
		case text = <-outCh:
		case <-time.After(readGrace):
