...
// searxClient — свой клиент с пределом, а не http.DefaultClient без предела:
// срок задаёт контекст, предел клиента — страховка (этап 91, R8.4).
var searxClient = &http.Client{Timeout: 60 * time.Second}
...
func (t *webSearchTool) Plan(args map[string]any) (*Plan, error) {
	...
	return &Plan{
		...
		Title: fmt.Sprintf("%s(%s)", NameWebSearch, textx.ShortenOneLine(query, 60)),
