...
type KB struct {
	...
	// Тощая книга: текст извлёкся, но его слишком мало для такого файла —
	// страницы-картинки в обёртке из разметки или превью издательства, где
	// от каждой главы взят один абзац. Такая книга в индекс не попадает:
	// в выдаче она даёт обрывки, а человек всё равно удаляет её руками
	// (23.09.2026 так пришлось удалить две). Числа выбраны замером на 559
	// книгах — подробности и таблица ложных срабатываний в kb.ThinLimits.
	// Отрицательное значение выключает признак, 0 — умолчание кода.
	// DedupeCosine — порог близости, выше которого кусок выдачи считается
	// повтором уже отобранного и место не занимает. Куски режутся
	// с перекрытием, и соседние куски одной книги делят по нескольку
	// предложений (на графе это уже дало 16 % подтверждений-копий).
	// 0 — не проверять; замер — этап 105, Ж4.
	DedupeCosine float64 `toml:"dedupe_cosine"`

	ThinMinChunks   int     `toml:"thin_min_chunks"`    // меньше кусков у файла крупнее thin_min_mb — отказ (0 — 100)
	ThinMinMB       int     `toml:"thin_min_mb"`        // с какого размера файла работает предел по кускам (0 — 3)
	ThinChunksPerMB float64 `toml:"thin_chunks_per_mb"` // плотность текста: меньше кусков на МБ — отказ (0 — 1.0)
	...
	// EmbedHeader — дописывать ли к куску шапку «название книги · с. N» перед
	// тем, как считать его вектор.
	//
	// **Умолчание — false, и это итог замера 09.09.2026** (этап 101, Г8).
	// Шапка стояла в коде с самого начала: фрагмент из середины главы без неё
	// беднее, чем на самом деле. Замер на золотом наборе (457 вопросов,
	// коллекция books, 533 тыс. кусков) показал, что пользы она не даёт:
	// с шапкой recall@10 векторов 0.339, без неё 0.346; MRR 0.225 против 0.228;
	// промахов 302 против 299. Разница — три вопроса из 457, то есть в пределах
	// шума, но в пользу «без шапки», а вектор куска при этом устроен так же,
	// как вектор вопроса (книга «Machine Learning Production Systems», 2024,
	// стр. 43–44 называет расхождение между ними training–serving skew).
	//
	// Смена этой настройки делает уже посчитанные векторы несогласованными:
	// доливка считала бы новые куски иначе, чем всю библиотеку. Поэтому она
	// записывается в паспорт векторов, а `--kb-doctor` о расхождении говорит.
	EmbedHeader    bool    `toml:"embed_header"`
...
type Mix struct {
	...
	// RelationYears — печатать у связей в карте понятий, уходящей модели, год
	// самой свежей подтверждающей книги: «(подтверждений 12, свежайшее 2021 г.)».
	// Человек в /search это видит с 16.09.2026, модель — нет; 17,1% связей графа
	// держатся только на книгах старше 2023 года (этап 104, П10). Умолчание —
	...
	// Abstain — привратник подмеса молчит, когда реранкер считает, что
	// в библиотеке об этом нет. Порог берётся тот же, что у kb_search
	// (kb.abstain_score −2 по замеру на 457 вопросах, kb.abstain_gap).
	//
	// Без этой проверки привратнику достаточно одного понятия с двумя
	// упоминаниями, а это ловится бытовыми словами: «какая погода в Москве
	// на выходных» цепляет понятие weather (в книгах про ИИ-агентов погодный
	// инструмент — стандартный пример) и получает карту про NetworkPolicy
	// и функции активации Keras (замер этапа 105, Ж6).
	//
	// Цена: в режиме «только карта» (mix.books = false) это лишний поиск
	// с реранкером на каждый вопрос — замер цены в Ж9. Умолчание false.
	Abstain bool `toml:"abstain"`

	// SenseEntry — входить в граф не только по написанию слов, но и по смыслу
	// (вектор вопроса). До 24.09.2026 подмешивание входило только по словам,
	// в отличие от /search и инструментов, и в карту приходило 2,2 понятия
	// при разрешённых шести (замер этапа 105, Ж6 и Ж10). Цена — вызов
	// эмбеддера перед каждым вопросом. Умолчание false.
	SenseEntry bool `toml:"sense_entry"`
...
type Graph struct {
	...
	// TripleQuota — сколько мест входа отдать тройкам ПЕРЕД смысловыми
	// понятиями (этап 105, В2.б). 0 — прежний порядок, и тогда тройкам
	// не достаётся ничего: замер В1 показал room = 0 и побайтно одинаковую
	// выдачу при triple_limit 0 и 3. Включается по замеру входа
	// (--graph-entry-eval) и судье по ответам, не «заодно».
	TripleQuota int `toml:"triple_quota"`
...
// ThinLimits собирает пределы «тощей» книги для индексации — так же, как
// Graph.Rules() собирает правила графа: пакет kb не знает о конфиге, числа
// приходят к нему параметром.
//
// Отрицательное значение в конфиге значит «не проверять этот признак» и
// доезжает до kb как -1; ноль означает «умолчание кода» и передаётся нулём.
func (k KB) ThinLimits() kb.ThinLimits {
	neg := func(v int) int {
		if v < 0 {
			return -1
		}
	...
	lim := kb.ThinLimits{MinChunks: neg(k.ThinMinChunks), MinMB: k.ThinMinMB, ChunksPerMB: k.ThinChunksPerMB}
	if lim.ChunksPerMB < 0 {
		lim.ChunksPerMB = -1
	}
	// Отрицательный thin_min_mb выключает признак «мало кусков», а не снимает
	// с него охрану по размеру: без неё предел по кускам ловил бы каждую
	// статью на 40 страниц, то есть значение «выключить» делало бы проверку
	// строже всего.
	if lim.MinMB < 0 {
		lim.MinChunks = -1
		lim.MinMB = 0
