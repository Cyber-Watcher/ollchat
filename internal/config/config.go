...
type Input struct {
	...
	// CollapsePaste — сворачивать ли большую вставку из буфера обмена в метку
	// вида «[Текст01: 4321 знак]». Порог — не число знаков, а место в поле
	// ввода: сворачивается то, что в него не помещается.
	//
	// При false вставка попадает в поле как есть. Так удобнее тем, кто хочет
	// видеть и править вставленное целиком, и неудобно тем, у кого поле низкое:
	// сто строк в трёхстрочном поле не видны ни началом, ни концом. Поэтому
	// настройка, а не общее решение (этап 100, слово владельца 07.09.2026).
	//
	// Свёрнутое разворачивается по F6 и сворачивается обратно по Shift+F6,
	// так что «true» ничего не прячет насовсем.
	CollapsePaste bool `toml:"collapse_paste"`
...
func Default() *Config {
	return &Config{
		...
		Input: Input{
			Mouse:         true,
			CollapsePaste: true, // прежнее поведение: большая вставка сворачивается
			Cursor: Cursor{
				Shape: CursorBlock,
				Blink: true,
				Color: "",
