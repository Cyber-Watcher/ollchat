...
func TestTemplateIsValid(t *testing.T) {
	...
	// Форма курсора в образце — вертикальная черта: её выбрал владелец, и её же
	// проверяем. Смысл проверки не в самой форме, а в том, что образец задаёт
	// форму явно и она доходит до настроек живой.
	if cfg.Input.Cursor.Shape != CursorBar {
		t.Errorf("в образце форма курсора = %q, ожидался %q", cfg.Input.Cursor.Shape, CursorBar)
	}
	// Дат в образце быть не должно: программа развивается, параметры в него
	// добавляются, а дата в комментарии стареет молча и вводит в заблуждение.
	if year := regexp.MustCompile(`\b20\d{2}\b`).FindString(Template); year != "" {
		t.Errorf("в образце конфига появилась дата (%s) — их там быть не должно", year)
	}
}

// chat_timeout отдельно от timeout: см. TimeOutPlan.md. Без него в
...
// Настройки ранжирования связей графа доходят из файла до структуры,
// а бессмысленные значения отклоняются с именем настройки.
//
// Проверка нужна потому, что ради этих двух чисел настройку и заводили:
// раньше они были константами в коде, и смена поведения требовала пересборки.
func TestGraphNeighborRankSettings(t *testing.T) {
	cfg, err := writeConfig(t, "[graph]\nneighbor_sense_weight = 1.5\nneighbor_pool = 8\n")
	if err != nil {
		t.Fatalf("загрузка конфига: %v", err)
	}
	if cfg.Graph.NeighborSenseWeight != 1.5 || cfg.Graph.NeighborPool != 8 {
		t.Errorf("настройки не применились: вес %v, пул %d",
			cfg.Graph.NeighborSenseWeight, cfg.Graph.NeighborPool)
	}

	// Умолчание — ноль: не пересортировывать. Замер 28.08.2026.
	def, err := writeConfig(t, "")
	if err != nil {
		t.Fatal(err)
	}
	if def.Graph.NeighborSenseWeight != 0 {
		t.Errorf("по умолчанию вес %v, ожидался 0", def.Graph.NeighborSenseWeight)
	}

	if _, err := writeConfig(t, "[graph]\nneighbor_sense_weight = -1\n"); err == nil {
		t.Error("отрицательный вес должен отклоняться: он поднял бы наверх самые далёкие от вопроса связи")
	}
	if _, err := writeConfig(t, "[graph]\nneighbor_pool = -3\n"); err == nil {
		t.Error("отрицательный пул должен отклоняться")
	}
}
...
// и он бы об этом не узнал.
func TestStartupHintsSetting(t *testing.T) {
	def, err := writeConfig(t, "")
	if err != nil {
		t.Fatal(err)
	}
	if !def.General.HintsAtStartup() {
		t.Error("по умолчанию советы при запуске должны показываться")
	}

	off, err := writeConfig(t, "[general]\nstartup_hints = \"off\"\n")
	if err != nil {
		t.Fatal(err)
	}
	if off.General.HintsAtStartup() {
		t.Error("startup_hints = off не применился")
	}

	if _, err := writeConfig(t, "[general]\nstartup_hints = \"of\"\n"); err == nil {
		t.Error("опечатка в значении должна отклоняться, иначе выключение молча не сработает")
	}
}

// Has различает «раздела нет в файле» и «настройки со значениями по умолчанию».
//
// По значению их не отличить: умолчания подставляются всем. А отличать нужно —
// по этому признаку прячутся команды ненастроенных возможностей.
func TestHasReportsSectionsWrittenInFile(t *testing.T) {
	withKB, err := writeConfig(t, "[kb]\ntop_k = 5\n")
	if err != nil {
		t.Fatal(err)
	}
	if !withKB.Has("kb") {
		t.Error("раздел [kb] написан в файле, а Has его не видит")
	}
	if withKB.Has("graph") {
		t.Error("раздела [graph] в файле нет, а Has говорит, что есть")
	}

	// Конфига нет вовсе — прятать нечего, показываем всё.
	fresh := Default()
	if !fresh.Has("kb") || !fresh.Has("graph") {
		t.Error("без файла настроек должны быть доступны все возможности")
	}
}

// Период проверки эмбеддера: умолчание, явное значение, ноль и опечатка.
func TestEmbedCheckSetting(t *testing.T) {
	def, err := writeConfig(t, "")
	if err != nil {
		t.Fatal(err)
	}
	if got := def.KB.EmbedCheckDuration(); got != time.Minute {
		t.Errorf("по умолчанию ожидалась минута, получено %v", got)
	}

	custom, err := writeConfig(t, "[kb]\nembed_check = \"30s\"\n")
	if err != nil {
		t.Fatal(err)
	}
	if got := custom.KB.EmbedCheckDuration(); got != 30*time.Second {
		t.Errorf("embed_check = 30s дал %v", got)
	}

	off, err := writeConfig(t, "[kb]\nembed_check = \"0\"\n")
	if err != nil {
		t.Fatal(err)
	}
	if got := off.KB.EmbedCheckDuration(); got != 0 {
		t.Errorf("«0» должен выключать повторные проверки, получено %v", got)
	}

	// Опечатка не должна молча превращаться в минуту: человек думал,
	// что настроил, а настройка не применилась.
	if _, err := writeConfig(t, "[kb]\nembed_check = \"минута\"\n"); err == nil {
		t.Error("недопустимое значение embed_check должно отклоняться при запуске")
	}
}

// Пороги и цвета сообщения об открытии графа: умолчания, свои значения
// и отказ на бессмысленных.
func TestGraphOpenThresholds(t *testing.T) {
	def, err := writeConfig(t, "")
	if err != nil {
		t.Fatal(err)
	}
	if def.Graph.OpenSlow() != 10*time.Second || def.Graph.OpenHot() != 20*time.Second {
		t.Errorf("умолчания = %v и %v, ожидались 10 с и 20 с", def.Graph.OpenSlow(), def.Graph.OpenHot())
	}
	if def.Graph.ColorOK() == "" || def.Graph.ColorSlow() == "" || def.Graph.ColorHot() == "" {
		t.Error("цвета по умолчанию должны быть заданы")
	}

	cfg, err := writeConfig(t, "[graph]\nopen_slow_seconds = 5\nopen_hot_seconds = 30\nopen_color_hot = \"#ff0000\"\n")
	if err != nil {
		t.Fatal(err)
	}
	if cfg.Graph.OpenSlow() != 5*time.Second || cfg.Graph.OpenHot() != 30*time.Second {
		t.Errorf("свои пороги не применились: %v, %v", cfg.Graph.OpenSlow(), cfg.Graph.OpenHot())
	}
	if cfg.Graph.ColorHot() != "#ff0000" {
		t.Errorf("свой цвет не применился: %q", cfg.Graph.ColorHot())
	}

	// Порог «долго» ниже порога «средне» — среднее состояние не показалось бы
	// никогда, и человек считал бы, что оно не работает.
	if _, err := writeConfig(t, "[graph]\nopen_slow_seconds = 20\nopen_hot_seconds = 10\n"); err == nil {
		t.Error("порядок порогов должен проверяться")
	}
	if _, err := writeConfig(t, "[graph]\nopen_color_ok = \"песочный\"\n"); err == nil {
		t.Error("недопустимый цвет должен отклоняться")
	}
}

// Кэш графа и показ памяти включены по умолчанию и выключаются настройкой.
func TestGraphCacheSettings(t *testing.T) {
	def, err := writeConfig(t, "")
	if err != nil {
		t.Fatal(err)
	}
	if !def.Graph.Cache || !def.Graph.ShowMemory {
		t.Error("по умолчанию кэш графа и показ памяти должны быть включены")
	}

	off, err := writeConfig(t, "[graph]\ncache = false\nshow_memory = false\n")
	if err != nil {
		t.Fatal(err)
	}
	if off.Graph.Cache || off.Graph.ShowMemory {
		t.Error("настройки не применились")
	}
}

// TestEmbedTimeoutErrorWraps — ошибка разбора kb.embed_timeout сохраняет
// исходную причину, а не только её текст.
func TestEmbedTimeoutErrorWraps(t *testing.T) {
	_, err := writeConfig(t, "[kb]\nembed_timeout = \"пятнадцать минут\"\n")
	if err == nil {
		t.Fatal("негодная длительность должна быть ошибкой")
	}
	inner := errors.Unwrap(err)
	if inner == nil || !strings.HasPrefix(inner.Error(), "time:") {
		t.Fatalf("исходная ошибка разбора потеряна: %v", err)
	}
}

// Ключ --steps-file называет готовый путь, в том числе абсолютный и вне
// каталога журналов; шаблон из настроек обязан быть относительным.
// 04.09.2026 замер инструментов упал на всех двадцати задачах, потому что
// ключ подставлялся в шаблон и не проходил его проверку.
func TestStepsFileOverridesPatternAndMayBeAbsolute(t *testing.T) {
	l := Log{StepsFilePattern: "steps-%Y.jsonl", StepsFile: "/tmp/probe/steps.jsonl"}
	p, err := l.StepsPattern()
	if err != nil {
		t.Fatal(err)
	}
	if got := p.Name(time.Now()); got != "/tmp/probe/steps.jsonl" {
		t.Fatalf("имя %q, ожидался путь из ключа", got)
	}
	if _, err := (Log{StepsFilePattern: "/tmp/steps.jsonl"}).StepsPattern(); err == nil {
