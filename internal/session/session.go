...
func (c *Conversation) Request() []ollama.Message {
	...
	if strings.TrimSpace(c.system) != "" {
		out = append(out, ollama.Message{Role: ollama.RoleSystem, Content: c.system})
	}
...
func (c *Conversation) Compact(keep int) int {
	...
	dropped := len(c.messages) - keep
	// Обрезаем так, чтобы история не начиналась с ответа инструмента без
	// соответствующего запроса — иначе модель получит рассогласованный диалог.
	for keep < len(c.messages) && c.messages[len(c.messages)-keep].Role == ollama.RoleTool {
		keep++
		dropped--
	}
	c.messages = append([]ollama.Message(nil), c.messages[len(c.messages)-keep:]...)
	return dropped
}
