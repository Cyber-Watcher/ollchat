...
func (c *Client) embedOnce(ctx context.Context, req EmbedRequest) (*EmbedResponse, error) {
	...
	if err != nil {
		...
		return nil, MarkRetryable(fmt.Errorf("запрос к %s: %w", c.baseURL, err))
	}
	defer httpResp.Body.Close()

	if httpResp.StatusCode != http.StatusOK {
		srvErr, body := serverError(httpResp)
		if httpResp.StatusCode >= 500 || runnerDied(body) {
			return nil, MarkRetryable(srvErr)
		}
		return nil, srvErr
	}
	var resp EmbedResponse
	if err := json.NewDecoder(httpResp.Body).Decode(&resp); err != nil {
		return nil, MarkRetryable(fmt.Errorf("разбор ответа /api/embed: %w", err))
	}
	return &resp, nil
}
