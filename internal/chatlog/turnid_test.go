...
// TestNewSessionIDFallback — отказ источника случайных чисел не роняет
// программу: идентификатор берётся из запасного источника и той же формы.
func TestNewSessionIDFallback(t *testing.T) {
	prev := randRead
	randRead = func([]byte) (int, error) { return 0, errors.New("нет энтропии") }
	t.Cleanup(func() { randRead = prev })

	id := NewSessionID()
	if len(id) != sessionIDLen {
		t.Fatalf("длина %d, ожидалось %d: %q", len(id), sessionIDLen, id)
	}
	for _, r := range id {
		if !strings.ContainsRune(idAlphabet, r) {
			t.Fatalf("знак %q вне алфавита: %q", r, id)
		}
	}
}
