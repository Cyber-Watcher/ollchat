...
// Выдержка показывает слово, найденное через перенос строки (правила v3).
//
// До 17.09.2026 выдержка резала текст на слова сама, иначе, чем индекс:
// кусок находился по «алгоритмы», а показывалось его начало, потому что
// в тексте выдержка видела только «алго» и «ритмы».
func TestSnippetFindsWrappedWord(t *testing.T) {
	filler := strings.Repeat("вводные слова без отношения к делу. ", 30)
	text := filler + "Здесь описаны популярные алго-\n    ритмы сортировки и поиска. " + filler
	got := Snippet(text, "алгоритмы сортировки", 200)
	if !strings.Contains(got, "алго-") || !strings.Contains(got, "сортировки") {
