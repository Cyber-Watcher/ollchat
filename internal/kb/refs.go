...
// уже начался. Заголовок — первая непустая строка куска, равная одному
// из refHeads целиком: «references» внутри предложения заголовком не считается.
//
// В признак FlagRefs входит с 17.09.2026 (этап 104, Ж1.2), после переписи:
// среди 318 тысяч разобранных кусков таких 125, из 14 просмотренных глазами
// 13 — настоящие «References» и «Further reading». Мало, но из них в граф
// приходили названия статей и фамилии, а в опытный граф им дороги нет вовсе.
...
// ServiceRefs — служебный ли это кусок рода «список литературы или выходные
// данные». Одна функция на оба места, где ставится FlagRefs (нарезка новых
// книг и перепрошивка признаков у собранных), чтобы правила не разошлись.
func ServiceRefs(text string) bool {
	return LooksLikeRefs(text) || LooksLikeColophon(text) || RefsHeading(text)
}
