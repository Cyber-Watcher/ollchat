...
func TestRealMerge(t *testing.T) {
	...
	start := time.Now()
	res, err := c.Merge(context.Background(), kb.MergeOpts{}, func(p kb.Progress) {
...
//
//	OLLCHAT_KB_LIVE=1 OLLCHAT_EMBED_URL=http://ollama.example:11434 \
//	OLLCHAT_EMBED_MODEL=bge-m3 go test ./internal/kb/live/ -run TestSemantic -v
//
// Условие приёмки записано в kb_semantic_search.md: русский запрос обязан
...
func TestSemanticCrossLanguage(t *testing.T) {
	...
	emb := kbembed.New(cfg.KB.EmbedOptions(), url, 10*time.Minute, nil)
	if emb == nil {
