...
//
// Удаления среди этих видов нет, и это не упущение: вид говорит, чем кончилось
// чтение файла, а удаление случается потом и относится не к тому же ключу.
// Запись реестра опознаётся по ПУТИ (loadDocs заменяет прежнюю запись того же
// пути), а удаление относится к НОМЕРУ книги — к её кускам, которые остаются
// лежать в chunks.dat. Положи по освободившемуся пути другой файл — и вид
// «удалена» в записи был бы затёрт, а куски прежней книги вернулись бы
// в выдачу. Поэтому удалённость живёт отдельным файлом-надгробием
// (deleted.ids), а спрашивать её надо у Book/BookAny, а не у Kind.
//
// Вид «gone» здесь был объявлен и НИКОГДА не использовался — след замысла,
// от которого отказались в пользу надгробий; убран 26.09.2026, чтобы его
// не принимали за рабочий признак (я сам принял).
...
type BookRec struct {
	...
	// Deleted — книга помечена удалённой (её номер в deleted.ids). В реестр
	// НЕ пишется (`json:"-"`): там запись удалённой книги остаётся нетронутой,
	// признак живёт отдельным файлом. Заполняется только чтением — BookAny.
	Deleted bool `json:"-"`

	// Hash — sha256 содержимого файла: ключ книги, не зависящий от номера.
	// Номера раздаёт индексация, и граф, ссылающийся на книги по номерам,
	// не переживает ни переиндексацию, ни уплотнение коллекции, ни переезд
	// (решение 6 паспорта опытного графа). По хешу граф переносится на новую
	// нумерацию (`--graph-rebase-books`). Считается при индексации (он и так
	// считается для поиска повторов) и командой `--kb-hash` для книг,
	// проиндексированных раньше 18.09.2026.
...
// и запись удалённой книги остаётся в `docs.jsonl` как была, с `kind: "ok"`
// и хешем. Удалённым книгу делает номер в отдельном файле `deleted.ids`.
// Поэтому «книга есть в реестре» и «книга жива» — разные вопросы, а из
// сигнатуры геттера этого не видно: 26.09.2026 перепись пропущенных кусков
// спросила первое, имея в виду второе, и записала 1 464 куска как готовые
// к перечитыванию, тогда как живых было 132 — остальные принадлежали
// удалённым книгам, которых нет и на диске.
//
// Отсюда умолчание: обычный вызов отдаёт только живые книги, а кому нужны
// и удалённые — пишет BookAny и получает признак в поле Deleted.
...
// BookAny находит книгу по номеру, ЖИВУЮ ИЛИ УДАЛЁННУЮ; у удалённой поле
// Deleted истинно. Нужна тому, кто разбирает старые журналы графа: в них
// остаются ссылки на книги, которых в коллекции уже нет.
