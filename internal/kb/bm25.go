...
type SearchOpts struct {
	...
	MaxPerDoc int // не больше стольких кусков из одной книги; 0 и меньше — без предела

	// KeepAdjacent — не выбрасывать соседние куски одной книги (этап 105, З5).
	//
	// **Зачем выключатель.** Нарезка идёт с перекрытием, и правило «не брать
	// кусок, если из этой книги уже взят соседний» спасает человека от одной
	// цитаты трижды. Но у замера он отнимает попадания: эталонный кусок
	// выбрасываем МЫ САМИ, когда его сосед оценён выше, а «мимо» выглядит как
	// промах поиска. Числа 24.09.2026: у 93 % промахов при k = 200 нужная книга
	// в выдаче есть, кусок рядом с эталоном — у половины, медиана расстояния 2.
	// Выключатель нужен, чтобы отделить одно от другого замером, а не спором.
	//
	// В работе умолчание прежнее: соседей выбрасываем.
	KeepAdjacent bool
...
func (s *searcher) Candidates(query string, opt SearchOpts) ([]Hit, error) {
	...
	// Служебный текст в выдачу не идёт: оглавления и указатели (FlagTOC,
	// этап 99) по словам совпадают с любым вопросом по книге, а ответа в них
	// нет; списки литературы и выходные данные (FlagRefs, этап 101) — перечень
	// чужих названий, из которого тоже ничего не узнать.
	for id := range scores {
		if s.store.Rec(id).Flags&uint16(FlagTOC|FlagRefs) != 0 {
			delete(scores, id)
		}
	}
	if len(scores) == 0 {
...
func (s *searcher) refine(hits []Hit, query string, opt SearchOpts) ([]Hit, error) {
	...
	for _, h := range head {
		...
		if !opt.KeepAdjacent && adjacentTaken(taken[doc], rec.Ord) {
			continue
		}
