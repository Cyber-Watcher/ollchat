...
// прилетает «сервер lab: ... ollama.example ... context deadline exceeded» —
...
func TestStaleServerErrorIsNotShownUnderNewName(t *testing.T) {
	...
	stale := m.gen.srv // поколение первого сервера

	m.switchServer("lab")

	m.Update(serverInfoMsg{gen: stale,
		err: errors.New(`GET /api/version: Get "http://ollama.example:11434/api/version": context deadline exceeded`)})

	for _, b := range m.blocks {
		if b.kind == blockError && strings.Contains(b.text, "ollama.example") {
...
func TestStaleServerInfoDoesNotOverwriteState(t *testing.T) {
	...
	m.switchServer("lab")
	m.Update(serverInfoMsg{gen: m.gen.srv, version: "0.32.7",
...
func TestStaleModelInfoRejectedEvenWithSameModelName(t *testing.T) {
	...
	m.Update(modelInfoMsg{gen: m.gen.srv, model: sameName,
...
func TestFreshServerInfoAccepted(t *testing.T) {
	...
	m.Update(serverInfoMsg{gen: m.gen.srv, version: "0.32.7", models: modelList("своя:latest")})
