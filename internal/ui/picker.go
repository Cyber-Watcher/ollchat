...
func (m *Model) switchServer(name string) tea.Cmd {
	...
	// Новый сервер — своя память карты: что было загружено на прежнем,
	// про этот не говорит ничего.
	m.residency = ollama.Residency{}
	m.saidLoad = false
	return tea.Batch(m.loadServerInfoCmd(), checkResidency(genStartup, m.client, m.modelName))
}
...
func (m *Model) switchModel(name string) tea.Cmd {
	...
	// Про новую модель прежний ответ не говорит ничего: загружена была та.
	m.residency = ollama.Residency{}
	m.saidLoad = false
	return tea.Batch(m.loadModelInfoCmd(), checkResidency(genStartup, m.client, name))
}

// resumeSession восстанавливает сохранённый диалог.
func (m *Model) resumeSession(id string) tea.Cmd {
	store := m.store
	// Чтение с диска — в фоне (этап 91, R6.6); в модель кладёт applyResumed.
	return func() tea.Msg {
		rec, err := store.Load(id)
		if err != nil {
			return errorMsg{err: fmt.Errorf("/resume: %w", err)}
		}
		return sessionLoadedMsg{rec: rec}
	}
}

// applyResumed заполняет диалог и ленту сохранённой сессией.
func (m *Model) applyResumed(rec *session.Saved) {
	m.conv.Restore(rec)
	m.blocks = nil
	m.rendered = nil
	m.meter.Reset()
