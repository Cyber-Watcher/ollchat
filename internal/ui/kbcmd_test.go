...
func TestKBAddIndexesAndSearches(t *testing.T) {
	...
	m.runCommand("/kb search goroutines -c go")
	if got := lastBlock(m).text; !strings.Contains(got, "go.pdf") || !strings.Contains(got, "стр.") {
...
func TestKBUseAndAuto(t *testing.T) {
	...
	if m.kbUse != "go" {
		t.Fatalf("коллекция не выбрана: %q", m.kbUse)
	}
	m.runCommand("/kb auto on")
	if !m.kbAutoOn {
	...
	// Теперь вопрос должен подмешать найденное.
	found, n := m.kbAutoContext("что такое goroutines")
	if n == 0 || !strings.Contains(found, "go.pdf") {
		t.Fatalf("подмешивание не сработало: %d фрагментов, %q", n, found)
	}
	...
	if _, n := m.kbAutoContext("что такое goroutines"); n != 0 {
		t.Fatal("подмешивание не выключилось")
	}
}
...
func drainJob(t *testing.T, m *Model, cmd tea.Cmd) {
	...
	gen, events := m.jobGen, m.job.events
