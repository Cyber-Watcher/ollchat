...
func (f *fileMenu) ensureVisible() {
	if f.cursor < f.offset {
		f.offset = f.cursor
	}
	if f.cursor >= f.offset+maxFileMenuRows {
		f.offset = f.cursor - maxFileMenuRows + 1
	}
	if f.offset < 0 {
		f.offset = 0
	}
...
func (f *fileMenu) move(delta int) {
	if len(f.entries) == 0 {
		return
	}
	f.cursor += delta
	if f.cursor < 0 {
		f.cursor = len(f.entries) - 1 // с начала списка — в конец
	}
