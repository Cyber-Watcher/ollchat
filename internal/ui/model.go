...
type Model struct {
	...
	// pasteSeq — сквозной счётчик вставок за сеанс: номер в метке обязан быть
	// однозначным, иначе история вернёт вопрос с чужим текстом (этап 100).
	pasteSeq int

	// pasteVault — все вставки сеанса по их меткам. Переживает отправку,
	// поэтому вопрос, возвращённый стрелкой вверх, снова знает свой текст.
	pasteVault map[string]string
