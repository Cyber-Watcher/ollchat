...
func (s scrollbar) offsetForRow(row int) int {
	...
	start := clampInt(row-size/2, 0, travel)
	return clampInt((start*s.maxOffset()+travel/2)/travel, 0, s.maxOffset())
}
