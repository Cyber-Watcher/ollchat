...
func (m *Model) resolvePDFPath(name string) (pdfTarget, error) {
	...
	name = addPDFExt(expandUserPath(name))
...
// expandUserPath раскрывает ведущий «~».
//
// Песочница умеет это сама, но через неё проходят только пути внутри корня,
// а «~/Документы/ответ.pdf» — как раз тот случай, ради которого команда
// разрешает выходить наружу.
...
func (m *Model) handleSavedPDF(msg savedPDFMsg) tea.Cmd {
	...
	if n := len(msg.missing); n > 0 {
		...
		text += fmt.Sprintf("\n  символов без глифа: %d (%s) — заменены на □",
			n, string(msg.missing[:minInt(n, 8)]))
	}
