...
func (p *imagePanel) ensureVisible() {
	if p.cursor < p.offset {
		p.offset = p.cursor
	}
	if p.cursor >= p.offset+maxImageRows {
		p.offset = p.cursor - maxImageRows + 1
	}
	if p.offset < 0 {
		p.offset = 0
	}
...
func (p *imagePanel) clampCursor(count int) {
	if count == 0 {
...
func (p *imagePanel) move(delta, count int) {
	if count == 0 {
		return
	}
	p.cursor += delta
