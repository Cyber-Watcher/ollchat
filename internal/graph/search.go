...
// Сколько подтверждений собирать на связь — Rules.MaxEvidences (умолчание 16).
//
// Под связью печатается одна выдержка, и эти записи — кандидаты на неё
// (evidenceLine, format.go). Проба 02.09.2026: восемь записей вместо четырёх
// не улучшили ни одной строки — и это верно, пока выдержкой берётся ПЕРВЫЙ
// кусок с обоими именами: у связи-долгожителя вроде `Go —использует→ Garbage
// collection` (186 подтверждений) первые записи идут подряд из одной книги.
// Замер 17.09.2026 показал, что решает не число кандидатов, а признак выбора:
// ближайшая пара имён даёт +11…15 п.п. выдержек, где связь видна.
...
func (g *Graph) pathByFlow(from, to uint32, maxHops, hubLimit int) ([]PathStep, bool) {
	...
	for h.Len() > 0 {
		...
		//
		// СЧИТАТЬ НАДО ТОЧНО ТАК ЖЕ, КАК ОБХОД ПО ШАГАМ (25.09.2026). Тот
		...
		// ДВАЖДЫ. Считать здесь уникальных соседей — значит мерить другой
		// линейкой: на узле, где половина связей двусторонние, число выходит
		// почти вдвое меньше, и порог 500 срабатывает у одного обхода и молчит
		// у другого. В замере Б5 от 20.09.2026 это дало 2 цепочки через хаб
		// (`scikit-learn`, 533 связи) там, где прежний обход не дал ни одной,
		// при записи в этапе «хабы в середине по-прежнему запрещены».
		// Поэтому здесь считаются ПАРЫ {сосед, направление} — в поле `dirs`
		// того же `arc`, без второй карты и без лишних выделений.
		// Пинается тестом TestChainHubLimitSameInBothPathModes.
		//
		// Верна ли сама мера (дубль по направлению — свойство хранения,
		// а не признак хаба) — отдельный вопрос: порог 500 замерен 08.09.2026
		// именно на ней, и смена меры требует перемера. Этап 105, Б5.
		type arc struct {
			edge Edge
			back bool
			w    float64
			dirs uint8 // 1 — есть исходящая связь, 2 — входящая
		}
		arcs := map[uint32]*arc{}
		total, degree := 0.0, 0
		...
		if total == 0 || (cur.id != from && hubLimit > 0 && degree >= hubLimit) {
			continue
		}
...
// emptyNode — понятие, которому нечего показать: ни упоминания в книгах,
// ни единой связи. Такое не предлагается смысловым входом.
//
// **Только смысловым.** По слову понятие находится, потому что человек назвал
// его имя — и получить в ответ «такое понятие есть, но про него ничего нет»
// честнее, чем молчание; так же ведут себя узлы, заведённые руками
// (`--graph-add-nodes`) до первой сборки. А по смыслу такой узел предлагает
// машина, и предлагать пустое ей незачем.
//
// **Откуда взялось.** Разбор одиночек 09.09.2026: из 10 997 понятий без связей
// 2 738 не имеют и упоминаний. Это следы чистки от оглавлений (этап 99):
// упоминания и подтверждения связей, извлечённые из оглавлений и указателей,
// были убраны из журналов, а сами понятия остались — номера понятий сдвигать
// нельзя, на них держатся векторы и разбиение.
//
// Порогом это не лечится: `MinMentions` в `/search` намеренно нулевой — человек
// ищет как раз редкое, и прятать от него понятие с одним упоминанием незачем.
