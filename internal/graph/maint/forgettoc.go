...
// ForgetTOC убирает из графа всё, что извлечено из кусков с признаком
// оглавления (этап 99). Признак ставит --kb-flag-toc; без него команде нечего
// забывать, и она так и скажет. Граф при этом закрыт, замок сборки свободен.
func ForgetTOC(stdout io.Writer, cfg *config.Config, name string, dry bool) error {
	...
	if len(toc) == 0 {
		return fmt.Errorf("в коллекции %s нет кусков с признаком оглавления — сначала ollchat --kb-flag-toc %s", name, name)
	}
