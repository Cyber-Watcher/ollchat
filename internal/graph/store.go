...
// around — связи узла в обе стороны, для обхода без оглядки на направление.
//
// Порядок устойчив: сперва исходящие, потом входящие. Иначе один и тот же
// вопрос давал бы разные цепочки от запуска к запуску.
func (e *Edges) around(id uint32) []Edge {
	out := e.outgoing(id)
	return append(append([]Edge(nil), out...), e.incoming(id)...)
}
