...
// **Что с синонимами, которые являются собственным именем другого понятия.**
// Это не «он же»: среди таких есть верные переводы («WaitGroup ←
...
// из 65 (aliases_sample.tsv, разбор в GraphHealth.md). До 04.09.2026 модели
// такие синонимы не показывались вовсе, человеку шли подряд с остальными.
// Теперь (этап 89, А1) обоим читателям они показываются **отдельной строкой
// с оговоркой** — «близкие понятия графа, не то же самое»: подсказка остаётся,
// подмены нет. В вектор понятия и в расширение запроса они по-прежнему
// не идут (Entities.SafeAliases). Поле RenderOpts.ForModel, которое якобы
// этим управляло, никто не читал и оно убрано 10.09.2026.
//
