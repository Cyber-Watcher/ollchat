...
type Communities struct {
	...
	// Blend — что дало подмешивание смысла в веса связей (opt.Beta).
	// На диск не пишется: это свойство того, **как** посчитано разбиение,
	// а не самого разбиения, и место ему в отчёте замера, а не в файле.
	Blend SenseBlend `json:"-"`
}
...
type CommunityOpts struct {
	...
	// Beta — насколько сильно смысл понятий влияет на вес связи (β).
	// Ноль — связи как есть, то есть прежнее поведение.
	//
	// Разбор — в internal/graph/senseblend.go: модулярность видит только вес
	// связи и не отличает «LLM → эмбеддинг» от «LLM → Notion», а векторы
	// понятий отличают. Это единственная ручка, дающая разбиению сигнал,
	// которого у него нет; γ таким сигналом не является — она меняет размер
...
func (g *Graph) partition(opt CommunityOpts, save bool) (*Communities, error) {
	...
	// Подмешанный смысл — измерительная ручка, а не режим работы: файл тем
	// о β не помнит (Blend не пишется), и разбиение, собранное по правленым
	// весам, лежало бы на диске неотличимым от обычного. Сравнивать с ним
	// следующий пересчёт было бы нечестно, а понять, откуда взялись темы, —
	// неоткуда. Решение владельца 06.09.2026: β в рабочее разбиение не входит.
	if save && opt.Beta > 0 {
		return nil, fmt.Errorf("разбиение с подмешанным смыслом (β = %.2f) на диск не пишется: "+
			"β — измерительная ручка --graph-tune-betas, а не режим работы", opt.Beta)
	}

	// Смысл подмешивается до разбиения и один раз: дробление крупных тем идёт
