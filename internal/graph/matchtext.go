...
// в hyphened на месте разрыва стоит пробел («специалистов практиков» — дефис
// внутри слова приравнен к пробелу и в тексте, и в имени).
...
func MatchText(text string) (joined, hyphened string) {
	...
	for i := 0; i < len(r); i++ {
		...
		case c == '\u00ad' || c == '\u2010' || c == '\u2011' || c == '\u2043' || c == '-':
			...
			// Дефис внутри слова равен пробелу — в тексте и в имени одинаково:
			// «load-balancing» и «load balancing», «KL-дивергенция» и
			// «KL дивергенция» — одно и то же. Перепись 17.09.2026: без этого
			// понятие с дефисом в имени «не находилось» в сотнях своих кусков.
			both(" ")
		case c == '_' || c == '/':
			both(" ")
		case (c >= '\u200b' && c <= '\u200d') || c == '\u2060' || c == '\ufeff':
