...
type Rules struct {
	...
	// понятиями (этап 105, В2.б). 0 — прежний порядок: слово, смысл, тройки.
	//
	// **Зачем выключатель.** Замер В1 (20.09.2026) показал две вещи разом:
	// индекс троек нужную связь находит (у пар со связью в графе искомая
	// X↔Y стоит первой тройкой у 55,9 % вопросов на books, в пятёрке 67,6 %),
	// но во входе ей не достаётся НИ ОДНОГО места — выдача при TripleLimit
	// 0 и 3 побайтно одинакова. Причина в порядке: смысловой вход забирает
	// свою половину раньше, и на тройки остаётся room = 0. То есть замер
	// «вход по тройкам не помогает» на самом деле мерил вход БЕЗ троек.
	//
	// Квота меняет только очередь за местами, не сам отбор: тройки берутся
	// тем же относительным правилом, что и при TripleLimit. Заданная квота
	// точна — половинное правило смыслового входа к ней не применяется,
	// потому что число выбирает тот, кто ставит замер.
	TripleQuota int
