...
// Порядок ночи и правила прогона — в docs/AItestingPlan.md.
//
...
func cmdGuard(ctx context.Context, args []string) error {
	...
	if !*quiet {
		...
		if rep.Free {
			fmt.Println("карта свободна — прогон можно начинать")
		} else {
...
func cmdReport(args []string) error {
	...
	fmt.Printf("%-32s %-12s %6s %6s %8s %8s %6s\n",
		"модель", "набор", "задач", "балл", "с/задачу", "ток/с", "сбоев")
	for _, k := range keys {
		s := sum[k]
		fmt.Printf("%-32s %-12s %6d %6.2f %8.0f %8.1f %6d\n",
			s.Model, s.Suite, s.Attempts, s.MeanScore, s.MedianSeconds, s.MedianTokPerSec, s.Errors)
	}
...
func cmdTimers(args []string) error {
	...
	var расписание strings.Builder
...
[Install]
WantedBy=timers.target
`, ConfigPath(c.Root), расписание.String(), c.Schedule.Timezone,
...
func cmdWindow(args []string) error {
	...
	case !*quiet:
		fmt.Printf("%s — %s %s–%s (%s), задачи раздаются до %s\n",
