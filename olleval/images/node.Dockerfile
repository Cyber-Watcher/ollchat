...
# Vite ищет пакеты обходом каталогов вверх от файла и `NODE_PATH` не смотрит
# вовсе: без этой ссылки `import { ref } from "vue"` в проверяемом коде падает
# с «Failed to load url vue». Ссылка в корне попадает в этот обход последней
# ступенью — сначала /w/node_modules, затем /node_modules.
RUN ln -s /opt/skel/node_modules /node_modules
