...
type GuardCfg struct {
	...
	// BusyUtilPercent — общий порог загрузки карты: столько процентов и ниже
	// считается простоем. Это умолчание для промежутков расписания, в которых
	// порог не указан явно ("00:00-07:00 gpu<=10").
	//
	// BusyVRAMMiB — занятая видеопамять. Прогон по ней **не** запрещается:
	// модель висит в памяти часами после чужого запроса, и по занятости стенд
	// выглядел бы вечно занятым. Правило включается только тогда, когда
	// загрузку измерить не удалось.
	BusyVRAMMiB     int `toml:"busy_vram_mib"`
	BusyUtilPercent int `toml:"busy_util_percent"`

	// UtilSamples и UtilSampleGap — сколько выборок загрузки брать за одну
	// проверку и с каким шагом. В расчёт идёт наибольшая: единственный снимок
	// nvidia-smi попадает в паузу между двумя токенами чужого ответа
	// и показывает ноль посреди работы.
	UtilSamples   int      `toml:"util_samples"`
	UtilSampleGap Duration `toml:"util_sample_gap"`
...
type RunCfg struct {
	...
	// CarryOver — насколько старую незаконченную ночь ещё имеет смысл
	// докатывать. Набор из сорока задач на восьми моделях не проходится
	// за сутки, и без докатки каждая полночь начинала бы его заново.
	// Предел нужен потому, что состав моделей на стенде меняется: докатывать
	// позапрошлую неделю бессмысленно, сравнивать её цифры будет не с чем.
	CarryOver Duration `toml:"carry_over"`
}
...
func DefaultConfig() Config {
	return Config{
		...
		Schedule: Schedule{
			...
			Days: DaySchedule{
				Monday:    []string{"00:00-07:00 gpu<=10"},
				Tuesday:   []string{"00:00-07:00 gpu<=10"},
				Wednesday: []string{"00:00-07:00 gpu<=10"},
				Thursday:  []string{"00:00-07:00 gpu<=10"},
				Friday:    []string{"00:00-07:00 gpu<=10"},
				Saturday:  []string{"00:00-23:59 gpu<=5"},
				Sunday:    []string{"00:00-23:59 gpu<=5"},
			},
		},
		Guard: GuardCfg{
			...
			BusyVRAMMiB: 1024, BusyUtilPercent: 10,
			UtilSamples: 5, UtilSampleGap: Duration(time.Second),
		...
		Run: RunCfg{
			...
			CarryOver: Duration(3 * 24 * time.Hour),
...
func (c Config) Validate() error {
	...
	case c.Guard.UtilSamples < 1:
		return fmt.Errorf("guard.util_samples = %d: нужна хотя бы одна выборка", c.Guard.UtilSamples)
	case c.Guard.BusyUtilPercent < 0 || c.Guard.BusyUtilPercent > maxSpanGPULimit:
		return fmt.Errorf("guard.busy_util_percent = %d: допустимо от 0 до %d",
			c.Guard.BusyUtilPercent, maxSpanGPULimit)
...
func (c Config) Get(key string) (string, error) {
	...
	case "guard.util_samples":
		return fmt.Sprint(c.Guard.UtilSamples), nil
	case "guard.util_sample_gap":
		return time.Duration(c.Guard.UtilSampleGap).String(), nil
...
#
# Рядом с промежутком можно поставить порог загрузки видеокарты: "gpu<=10"
# означает «десять процентов и ниже считаем простоем». Порог нужен потому,
# что модель остаётся в видеопамяти часами после чужого запроса: если ждать
# пустой карты, ночь не начнётся никогда. Допустимо от 0 (строго ноль)
# до 20 процентов; без порога берётся общий guard.busy_util_percent.
#
#   monday = ["00:00-07:00", "13:00-14:00"]   — ночь и обеденный перерыв
#   friday = []                                — по пятницам не гоняем
[schedule.days]
monday    = ["00:00-07:00 gpu<=10"]
tuesday   = ["00:00-07:00 gpu<=10"]
wednesday = ["00:00-07:00 gpu<=10"]
thursday  = ["00:00-07:00 gpu<=10"]
friday    = ["00:00-07:00 gpu<=10"]
saturday  = ["00:00-23:59 gpu<=5"]
...
check_gpu      = true   # загрузка видеокарты и процессы на ней (nvidia-smi)
check_ps       = true   # загруженные модели (/api/ps) — при простое только к сведению
...
# Старт решает загрузка карты, а не занятая на ней память: чужой процесс или
# забытая модель могут держать десятки гигабайт сутками, ничего при этом
# не считая. Загрузка меряется серией выборок, в расчёт идёт наибольшая —
# один снимок nvidia-smi попадает в паузу между токенами чужого ответа.
busy_util_percent = 10     # порог по умолчанию, если в промежутке не задан свой
util_samples      = 5      # выборок загрузки за одну проверку
util_sample_gap   = "1s"   # шаг между выборками

# Занятая видеопамять прогон не запрещает. Правило включается только тогда,
# когда загрузку измерить не удалось (нет nvidia-smi, непонятный ответ):
# в этом случае старт запрещают и процессы на карте, и занятая память,
# и загруженная модель — слепой старт хуже пропущенной ночи.
busy_vram_mib     = 1024
...
carry_over = "72h"      # насколько старую незаконченную ночь ещё докатывать:
                        # набор из сорока задач не проходится за сутки, и без
                        # докатки каждая полночь начинала бы его заново
