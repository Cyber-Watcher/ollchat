...
`
	...
	if cfg.URL != "http://ollama.example:11434" {
	...
	if got := cfg.Schedule.Days.Sunday; len(got) != 1 || got[0] != "00:00-23:59 gpu<=5" {
...
func TestLoadConfigWithoutFile(t *testing.T) {
	...
	if got := cfg.Schedule.Days.Monday; len(got) != 1 || got[0] != "00:00-07:00 gpu<=10" {
		t.Errorf("умолчание расписания = %v", got)
	}
}

// ConfigValidate ловит ошибки.
...
func TestConfigGet(t *testing.T) {
	...
	for key, want := range map[string]string{
		"schedule.days.monday": "00:00-07:00 gpu<=10", "schedule.days.saturday": "00:00-23:59 gpu<=5",
...
// WriteConfigTemplate не затирает готовый.
func TestWriteConfigTemplateKeepsExisting(t *testing.T) {
	path := filepath.Join(t.TempDir(), "olleval.toml")
	if err := WriteConfigTemplate(path, "/home/runner/ollevals"); err != nil {
		t.Fatal(err)
	}
	cfg, err := LoadConfig(path)
	if err != nil {
	...
	if got := cfg.Schedule.Days.Monday; len(got) != 1 || got[0] != "00:00-07:00 gpu<=10" || cfg.Guard.FreeChecks != 3 {
...
func TestWatchScheduleSeesLiveEdit(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "olleval.toml")
...
func TestWatchScheduleSurvivesBrokenFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "olleval.toml")
